...
void ShelterFiftyFiftyLoopFunction::PostStep()
{
  ...
  for (CSpace::TMapPerType::iterator it = tEpuckMap.begin(); it != tEpuckMap.end(); ++it)
  {
    ...
    if (m_cShelterLeftX.WithinMinBoundIncludedMaxBoundIncluded(cEpuckPosition.GetX()) and m_cShelterLeftY.WithinMinBoundIncludedMaxBoundIncluded(cEpuckPosition.GetY()))
    {
      unNbRobotLeft += 1;
    }
    if (m_cShelterRightX.WithinMinBoundIncludedMaxBoundIncluded(cEpuckPosition.GetX()) and m_cShelterRightY.WithinMinBoundIncludedMaxBoundIncluded(cEpuckPosition.GetY()))
    {
      unNbRobotRight += 1;
    }
  }
