...
void GiandujaNestingLoopFunction::PostStep()
{
  ...
  for (CSpace::TMapPerType::iterator it = tEpuckMap.begin(); it != tEpuckMap.end(); ++it)
  {
    ...
    if (m_unState == 0)
    {
      ...
      else if ((cEpuckPosition.GetX() <= m_CCoordRect1.GetX()) && (cEpuckPosition.GetX() >= m_CCoordRect2.GetX()) && (cEpuckPosition.GetY() >= m_CCoordRect2.GetY()) && (cEpuckPosition.GetY() <= m_CCoordRect1.GetY()))
      { // inside nest
        m_unCostI += 1;
      }
      m_unTbar += 1;
